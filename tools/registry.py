...
def memory_recall(query: str, *, _memory=None) -> str:
    ...
    _memory 是隐藏参数：由 Agent 在注册时用 functools.partial 绑定 MemoryManager 实例，
    不暴露给 LLM，模型只会传 `query`。
    """
    if _memory is None:
        return "【记忆系统未初始化】请联系管理员检查 MemoryManager 配置。"

    results = _memory.search(query, threshold=0.0, top_k=5)
    if not results:
        return f"【未检索到相关记忆】没有与「{query}」直接相关的历史记忆。"

    lines = [f"📝 用户记忆检索结果（query=「{query}」）：", ""]
    for i, r in enumerate(results, 1):
        lines.append(f"[记忆{i}] 相关度 {r['score']:.2f}")
        lines.append(r["content"])
        lines.append("")
    lines.append("——以上为完整记忆。回答时请基于这些信息给出个性化建议。")
    return "\n".join(lines)
...
MEMORY_RECALL_SCHEMA = {
    "name": "memory_recall",
    "description": (
        "检索用户的持久记忆，返回与查询相关的完整记忆原文。"
        "触发时机：当系统提示中的「用户记忆摘要」出现了与当前问题明确相关的条目，"
        "但摘要信息不够详细（如只知道用户收入但不知道具体目标、只知道偏好但不知道风险承受度），"
        "需要完整上下文来给出更个性化建议时调用。"
        "负向约束：①不要无目的地检索——记忆摘要与当前问题明显无关时不要调用；"
        "②不要用它做数学计算（那是 calculate 的职责）；"
        "③不要用它查文档（那是 retrieve_document 的职责）；"
        "④不要用它查汇率/基金（那是 get_exchange_rate / get_fund_info 的职责）；"
        "⑤纯通用理财常识无需检索，直接回答即可。"
    ),
    "input_schema": {
        "type": "object",
        "properties": {
            "query": {
                "type": "string",                "description": (
                    "检索用的自然语言查询，尽量贴近想回忆的信息。"
                    "如『用户的月收入、买房目标和当前储蓄情况』"
                ),
            }
        },
        "required": ["query"],
...
TOOL_REGISTRY: dict[str, Callable[..., Any]] = {
    ...
    "retrieve_document":        retrieve_document,   # ★ Day 1-2：文档检索工具
    "memory_recall":            memory_recall,        # ★ Day 3-4：记忆检索工具
}
...
TOOL_SCHEMAS = [{
    ...
    "input_examples": [
        {"transactions_json": "[{\"category\":\"餐饮\",\"amount\":500,\"description\":\"周末聚餐\"},{\"category\":\"交通\",\"amount\":200,\"description\":\"地铁充值\"},{\"category\":\"工资\",\"amount\":-8000,\"description\":\"月薪\"}]", "period": "2024年12月"},
    ...
    "input_examples": [
        {"monthly_income": 15000, "monthly_expense": 12000, "total_savings": 50000, "monthly_debt_payment": 2000},
        {"monthly_income": 8000, "monthly_expense": 7500, "total_savings": 10000}
    ]
}, {
    ...
    "input_examples": [
        {"monthly_income": 12000, "financial_goal": "买房", "current_obligations": "房租2500"},
        {"monthly_income": 9000, "financial_goal": "平衡储蓄与生活质量"},
        {"monthly_income": 15000, "financial_goal": "还债", "current_obligations": "房贷4000+车贷1500"}
    ]
}, {
    ...
    "input_examples": [
        ...
        {"from_currency": "CNY", "to_currency": "JPY"},
    ...
    MEMORY_RECALL_SCHEMA,   # ★ Week1 Day3-4：记忆检索工具
]
